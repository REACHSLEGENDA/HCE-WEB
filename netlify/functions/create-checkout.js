...
export const handler = async (event) => {
  ...
  try {
    ...
    const mxnToUnit = (mxn, isBase = false) => {
      ...
      const finalMXN = isBase ? precioConPromo(mxn, promoCode, 'paris', now) : mxn;
    ...
    const discountedBase = precioConPromo(baseAmount, promoCode, 'paris', now);
    ...
    const enableInstallments = habilitaMeses(promoCode, 'paris', now);
