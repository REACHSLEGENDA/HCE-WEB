...
export const getFlagUrl = (countryName) => {
  ...
  // Try exact match or match ignoring accents
  const match = COUNTRIES.find(c => {
    ...
    return nameClean === cleanNoAccents || c.name.toLowerCase() === clean;
  });

  if (match) {
    return `https://flagcdn.com/w40/${match.code}.png`;
  }
  ...
  const partialMatch = COUNTRIES.find(c => {
    const nameClean = c.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    const cleanNoAccents = clean.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    return nameClean.includes(cleanNoAccents) || cleanNoAccents.includes(nameClean);
  });

  if (partialMatch) {
    return `https://flagcdn.com/w40/${partialMatch.code}.png`;
  }
