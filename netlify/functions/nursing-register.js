...
export const handler = async (event) => {
  ...
  try {
    ...
    // Quitar tag de abandono y refrescar tag de éxito
    ...
    // Pequeña espera para que Mailchimp procese el cambio
    await new Promise(resolve => setTimeout(resolve, 1500));

    // Volver a poner el tag (activo)
    await fetch(`${baseUrl}/members/${hash}/tags`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
    ...
    console.error('Mailchimp register error:', err.message);
    return {
      statusCode: 500,
